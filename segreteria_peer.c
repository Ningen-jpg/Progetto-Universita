...
{
    int buff;
    if ((read(connfd, &buff, 4)) < 0) // legge la chiave da cercare (mandata da studente)
    {
        perror("errore read");
        exit(1);
    }
    ...
    while (1)
    {
        ...
        if (FD_ISSET(listenFD, &readSet))
        {
            ...
            { // SE SONO IL FIGLIO GESTISCO IL SERVZIO
                ...
                read(connectFD, &choice, 4); // connectFD è fd della connessione con studente
                ...
                case 1: // invia
                {
                    ...
                    // invio la chiave al server per la ricerca
        ...
        if (FD_ISSET(STDIN_FILENO, &readSet))
        {
            ...
            if (read(socketClientFD, readBuf, sizeof(readBuf)) > 0)
            { // LEGGO
